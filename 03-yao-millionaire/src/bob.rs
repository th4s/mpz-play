...
async fn main() {
    ...
    let input_alice = garble_vm.new_blind_input::<u32>("input_alice").unwrap();
    let input_bob = garble_vm.new_private_input::<u32>("input_bob").unwrap();

    let output_alice = garble_vm.new_output::<bool>("output_alice").unwrap();
    let output_bob = garble_vm.new_output::<bool>("output_bob").unwrap();

    // Assign the input.
    garble_vm.assign(&input_bob, 2_000_000_u32).unwrap();
    ...
    garble_vm.decode_blind(&[output_alice]).await.unwrap();
    let mut output = garble_vm.decode_private(&[output_bob]).await.unwrap();
